...
class SHGHSFeedbackSupabaseUser extends BaseAuthUser {
  SHGHSFeedbackSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sHGHSFeedbackSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SHGHSFeedbackSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
